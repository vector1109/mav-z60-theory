# Documento 3 — Teorema general de clasificación Φ en ℤₙ
...
- Toda órbita converge a uno de estos dos estados
...
Para todo \(x \in \mathbb{Z}_n\), existe \(m \in \mathbb{N}\) tal que:

\[
f^m(x) = \Phi(x)
\]

o pertenece a una órbita periódica contenida en la fibra de \( \Phi(x) \).
...
Cada fibra es una cuenca de atracción.

---

## 7. Interpretación estructural

El operador Φ actúa como:

> Proyección global del sistema dinámico hacia su esqueleto idempotente

Reduce la dinámica compleja a un sistema discreto de estados finales.
...
- Todas las órbitas colapsan en profundidad ≤ 2
...
donde Φ es la estructura de colapso fundamental del sistema.